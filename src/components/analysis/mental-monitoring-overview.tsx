...
import type { MentalMonitoringResult } from "@/lib/types";
import { Heart, AlertTriangle, CheckCircle, Info, ShieldCheck } from "lucide-react";

interface MentalMonitoringOverviewProps {
  monitoring: MentalMonitoringResult;
}
...
const LEVEL_CONFIG = {
  none: {
    ...
    description: "No mental health signals were detected in this work.",
  },
  mild: {
    icon: Info,
...
export function MentalMonitoringOverview({ monitoring }: MentalMonitoringOverviewProps) {
  const config = LEVEL_CONFIG[monitoring.level];
      ...
      <CardContent className="space-y-4">
        ...
        <div className="space-y-2">
          ...
          <div className="flex gap-1.5">
            <div className={`h-3 flex-1 rounded-l-full ${monitoring.level !== "none" ? "bg-green-500" : "bg-green-500"}`} />
            <div className={`h-3 flex-1 ${monitoring.level === "mild" || monitoring.level === "flag" ? "bg-yellow-500" : "bg-muted"}`} />
            <div className={`h-3 flex-1 rounded-r-full ${monitoring.level === "flag" ? "bg-red-500" : "bg-muted"}`} />
          </div>
        ...
        <div className="space-y-2">
          ...
          <p className="text-sm leading-relaxed bg-muted/50 rounded-lg p-3">{monitoring.note}</p>
        </div>
        ...
        <div className="space-y-2">
          ...
          <div className="flex items-start gap-2 bg-muted/50 rounded-lg p-3">
            ...
            <p className="text-sm leading-relaxed">{monitoring.next_step}</p>
          </div>
        </div>
        ...
        <p className="text-[11px] text-muted-foreground italic border-t pt-3">
          ...
          All decisions regarding student mental health must be made by qualified professionals.
        </p>
      </CardContent>
    </Card>
  );
